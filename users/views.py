...
def register(request):
    if request.method == 'POST':
        ...
        if request.POST['password1'] == request.POST['password2']:
            ...
            return render(request, 'users/register.html', {'error':'Passwords must match'})
    else:
...
def brazilian(request):
    return render(request, 'users/brazilian.html')

def starbeans(request):
    return render(request, 'users/starbeans.html')